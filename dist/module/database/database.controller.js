...
class DatabaseController {
    ...
    async deleteBackup(req, res) {
        ...
        try {
            ...
            res.status(500).json({ message: "Failed to delete backup", error: err });
        }
    }
    // Renombrar un respaldo
    async renameBackup(req, res) {
        const { filename } = req.params;
        const { newName } = req.body;
        if (!newName || !newName.endsWith(".sql")) {
            return res.status(400).json({ message: "New name must be a .sql file" });
        }
        const oldPath = path_1.default.join(this.backupsDir, filename);
        ...
        try {
            ...
            res.status(200).json({ message: "Backup renamed", newName });
        }
        catch (err) {
            res.status(500).json({ message: "Failed to rename backup", error: err });
        }
    }
    // En DatabaseController
    async restoreFromFile(req, res) {
        ...
        const restoreCommand = `mysql -h ${envs_1.DB_HOST} -P ${envs_1.DB_PORT} -u ${envs_1.DB_USERNAME} -p${envs_1.DB_PASSWORD} ${envs_1.DB_NAME} < ${filePath}`;
        (0, child_process_1.exec)(restoreCommand, (error) => {
            if (error) {
                console.error("❌ Error during restore:", error);
                return res.status(500).json({ message: "Restore failed", error });
    ...
    async downloadFromFile(req, res) {
        const { filename } = req.params;
        if (!filename || !filename.endsWith(".sql")) {
